    ...
    <div class="container mx-auto px-4">
        <div
            ...
            <a href="/" class="flex items-center gap-2 group">
                ...
                <span class="text-2xl font-bold tracking-tight text-white">ShopLink</span>
            </a>
            ...
            <div class="hidden md:flex flex-grow max-w-xl relative">
                <form action="#" class="w-full relative">
                    <input type="text"
                        class="w-full pl-6 pr-12 py-3 rounded-full text-sm text-gray-700 outline-none border-none focus:ring-0 placeholder-gray-400 bg-white shadow-sm"
                        placeholder="Search for Grocery, Stores, Vegetable or Meat">
                    <button type="button"
                        class="absolute right-2 top-1/2 -translate-y-1/2 text-gray-500 hover:text-primary transition p-2">
            ...
            <div class="flex items-center gap-6 text-white text-sm font-medium">
                <div class="hidden xl:flex items-center gap-2 text-accent">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2"
                        stroke="currentColor" class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" />
                    </svg>
                    <span>Order now and get it within <span class="font-bold">15 mint!</span></span>
                </div>

                <!-- Cart -->
                @if (!auth()->check() || !auth()->user()->isSeller())
                    <a href="{{ route('cart.index') }}"
                        class="relative cursor-pointer bg-white text-primary p-2 rounded-full hover:bg-gray-100 transition">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                        ...
                        @auth
                            ...
                            @if ($cartCount > 0)
                                <span
                                    class="cart-count-badge absolute -top-1 -right-1 bg-red-500 text-white text-[10px] font-bold h-5 w-5 flex items-center justify-center rounded-full border-2 border-primary">
                                    {{ $cartCount }}
                                </span>
                            @endif
                        @endauth
